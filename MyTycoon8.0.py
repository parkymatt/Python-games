...
class TycoonGame:
    ...
    def auto_buy_businesses(self):
        ...
        upcoming_tax = int(self.cash * 0.10)
        upcoming_wages = sum(w["hired"] * w["wage"] for w in self.workers.values())
        reserve_buffer = upcoming_tax + upcoming_wages + 1000  # buffer to avoid bankruptcy

        available_cash = self.cash - reserve_buffer
        if available_cash <= 0:
            print("🤖 Managers decided to save money for taxes and wages.")
            return

        # Sort businesses by ROI (income / cost) descending
        business_choices = []
        for name, b in self.businesses.items():
            roi = b["income"] / b["cost"]
            business_choices.append((roi, name))

        business_choices.sort(reverse=True)

        bought_any = False
        for roi, name in business_choices:
            business = self.businesses[name]
            while available_cash >= business["cost"]:
                self.cash -= business["cost"]
                available_cash -= business["cost"]
                business["owned"] += 1
                print(f"🤖 Manager bought 1 {name} for ${business['cost']}")
                bought_any = True

        if not bought_any:
            print("🤖 Managers decided not to buy anything this round.")
